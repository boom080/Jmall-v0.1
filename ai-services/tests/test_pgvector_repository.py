...
def test_database_ingest_writes_pgvector_embedding_column():
    ...
    chunk_inserts = [call for call in connection.calls if "insert into jrunmall_merchant_ai.knowledge_chunks" in call[0]]
...
def test_pgvector_migration_only_rebuilds_rag_tables():
    script = Path(__file__).resolve().parents[2] / "resource" / "db" / "5.7-rag-pgvector-1024.sql"
