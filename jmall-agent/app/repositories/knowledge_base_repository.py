...
class KnowledgeBaseRepository:
        ...
        if self.engine is not None:
            with self.engine.begin() as connection:
                result = connection.execute(
                    text(f"delete from {self.schema}.knowledge_bases where id = :id"),
                    {"id": knowledge_base_id},
                )
            return bool(result.rowcount)

        state = self._state()
        original_count = len(state["knowledge_bases"])
        state["knowledge_bases"] = [
            item for item in state["knowledge_bases"]
            if str(item.get("id")) != knowledge_base_id
        ]
        if len(state["knowledge_bases"]) == original_count:
            return False
        state["documents"] = [
            item for item in state["documents"]
            if str(item.get("knowledgeBaseId")) != knowledge_base_id
        ]
        state["chunks"] = [
    ...
    def create_knowledge_base(
        ...
        for item in self._visible_knowledge_bases(state["knowledge_bases"]):
            item_name = str(item.get("name") or item.get("label") or "").strip().casefold()
            if item_name == normalized_name:
                item_id = str(item["id"])
                return KnowledgeBaseCreateResponse(
                    id=item_id,
                    label=str(item.get("label") or item.get("name") or name),
                    description=str(item.get("description") or ""),
                    embeddingStatus=self._knowledge_base_embedding_status(item_id),
    ...
    def _list_knowledge_bases_from_database(self) -> List[KnowledgeBaseOptionResponse]:
        sql = text(
            f"""
            with ranked_knowledge_bases as (
                select kb.*,
                       row_number() over (
                           partition by lower(btrim(coalesce(nullif(kb.name, ''), nullif(kb.label, ''), kb.id)))
                           order by kb.updated_at desc, kb.created_at desc, kb.id
                       ) as name_rank
            ...
            from ranked_knowledge_bases kb
    ...
    def _create_knowledge_base_in_database(
        ...
        existing_sql = text(
            f"""
            select id, coalesce(nullif(label, ''), name) as label, description, source, updated_at
            from {self.schema}.knowledge_bases
            where lower(btrim(coalesce(nullif(name, ''), label))) = lower(btrim(:name))
            order by updated_at desc, created_at desc
            limit 1
            """
        )
        with self.engine.connect() as connection:
            existing_rows = connection.execute(existing_sql, {"name": normalized_name}).mappings().all()
        ...
        if existing is not None:
            return KnowledgeBaseCreateResponse(
                ...
                embeddingStatus="existing",
                source=str(existing["source"] or source),
                updatedAt=self._format_datetime(existing["updated_at"]),
            )
