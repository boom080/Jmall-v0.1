...
public class SkuInfoServiceImpl extends ServiceImpl<SkuInfoDao, SkuInfoEntity> implements SkuInfoService {
    ...
    public PageUtils queryPageCondition(Map<String, Object> params) {
        ...
        QueryWrapper<SkuInfoEntity> queryWrapper = new QueryWrapper<SkuInfoEntity>().orderByDesc("sku_id");

        Object publishedOnly = params.get("publishedOnly");
        if (publishedOnly != null && Boolean.parseBoolean(String.valueOf(publishedOnly))) {
            queryWrapper.apply("(spu_id IS NULL OR spu_id IN (SELECT id FROM pms_spu_info WHERE publish_status IS NULL OR publish_status <> 0) OR spu_id NOT IN (SELECT id FROM pms_spu_info))");
        }
